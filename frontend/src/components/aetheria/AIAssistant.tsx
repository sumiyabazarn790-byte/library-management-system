...
import { MONGOLIAN_CAPABILITIES_REPLY, resolveLocalAssistantReply } from "@/lib/assistant";
...
export const AIAssistant = () => {
  ...
  const [messages, setMessages] = useState<Message[]>([
    {
      role: "assistant",
      content: MONGOLIAN_CAPABILITIES_REPLY,
    },
  ]);
  ...
  return (
    <>
      <button
        ...
        className={`fixed bottom-6 right-6 z-50 flex h-14 items-center gap-2 rounded-full bg-gradient-accent px-5 text-sm font-semibold text-primary-foreground shadow-glow-primary transition-all hover:scale-105 ${open ? "hidden" : ""}`}
      >
      ...
      {open && (
        <div className="fixed bottom-6 right-6 z-50 flex h-[min(680px,calc(100vh-3rem))] w-[min(440px,calc(100vw-2rem))] flex-col overflow-hidden rounded-2xl glass-strong ring-hairline-strong shadow-cinematic animate-fade-up">
          <header className="flex h-14 items-center justify-between border-b border-border/40 px-5">
            <div className="flex items-center gap-2.5">
              ...
              <div>
                <p className="font-display text-sm font-semibold leading-none">Aetheria AI</p>
                <p className="mt-1 text-[10px] uppercase tracking-wider text-muted-foreground">Multilingual Librarian</p>
              </div>
            </div>
            <button onClick={() => setOpen(false)} className="rounded-md p-2 text-muted-foreground hover:bg-surface-high hover:text-foreground">
              <X className="size-4" />
            </button>
          </header>
            ...
            {messages.map((message, index) => (
              <div key={index} className={`flex ${message.role === "user" ? "justify-end" : "justify-start"}`}>
                <div className={`flex items-end gap-2 ${message.role === "user" ? "flex-row-reverse" : ""}`}>
                  {message.role === "assistant" ? <AssistantAvatar className="size-9 shrink-0" /> : null}
                  <div
          ...
          <div className="flex gap-2 border-t border-border/40 p-3">
            <input
              ...
              className="h-11 flex-1 rounded-md border border-border bg-surface-elevated px-4 text-sm transition-colors focus:border-primary focus:outline-none"
            />
            <button
              onClick={() => void send()}
              disabled={streaming || !input.trim()}
              className="flex h-11 w-11 items-center justify-center rounded-md bg-primary text-primary-foreground shadow-glow-primary transition-all hover:shadow-[0_0_50px_hsl(var(--primary)/0.55)] disabled:opacity-50 disabled:shadow-none"
            >
